...
- **文件**：
  ...
  - `lib/follow-up-service.ts`：两处触发源头 gate——`scheduleFollowUp` 创建侧（追发的唯一入口，覆盖全部调用方）与 `fireTimedWake` 开火侧（定时唤醒）
  - `components/chat/chat-settings-panel.tsx`：聊天信息页「置顶聊天」下方新增「允许主动消息」开关；关闭时同步清掉该会话已排队的追发/定时唤醒计划
- **原因**：float 的主动消息（追发/定时唤醒）没有任何按角色的控制。用户玩同人卡，
  大量非重要 NPC 好友频繁主动要求做事/见面造成压力；但主要角色仍希望保留主动消息，
  不能全局一刀切（追发的触发器是焦虑值状态数字，与预设条目解耦，关提示词条目
  无法阻止 API 调用——见调查结论）。
- **实现要点**：gate 全部设在**创建侧/开火前**，不留幽灵 API 调用；经期关怀有自己的
  角色名单配置，不纳入本开关。群聊会话同样可用（掐的是会话级 schedule）。
- **合并提示**：若上游自己实现了按角色/会话的主动消息控制，按上游优先原则采用上游版
  并删除本条（注意确认上游是否同时 gate 了追发的焦虑值路径——FAQ 目前教的关条目方案
  并不能真正阻止追发调用）。
