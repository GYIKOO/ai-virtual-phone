...
export async function armFollowUpBailout(
    ...
    try {
        ...
        if (session.proactiveDisabled) return; // 会话级开关：不预约服务端兜底
...
export async function armIdleReconnectBailout(rule: IdleReconnectRule): Promise<BailoutArmResult> {
    ...
    try {
        ...
        if (session?.proactiveDisabled) return { ok: false, reason: "该会话已关闭主动消息" }; // 会话级开关：不预约服务端兜底
    ...
    try {
        ...
        for (let dayOffset = 0; dayOffset <= 7; dayOffset += 1) {
            ...
            const sessions = loadChatSessions()
                // 会话级「允许主动消息」关闭的会话不预约服务端兜底（与本地 fireMenstrualPeriodCare 的 gate 对齐）
                .filter(session => !session.isGroup && !session.proactiveDisabled && config.periodCareCharacterIds.includes(session.contactId))
